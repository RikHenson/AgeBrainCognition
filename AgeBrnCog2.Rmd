...
Note that you get identical model fits with other parametrisations of this model:
```{r CS lme age DoA other}
# 1. model DoA rather than Age
s <- summary(lmer('B ~ DoB + DoA + (1|id)', data = dfl)); s$call; s$coefficients

#2. reparametrise as age at baseline (bA) and difference in age across assessments (dA):
...
#    B1 ~~ B1; B2 ~~ B2; B3 ~~ B3
'            
SEM8_LCMB_dfw <- lavaan::growth(SEM8_LCMB, data = dfw)
#SEM8_LCMB_dfw <- sem(SEM8_LCMB, data = dfw)
summary(SEM8_LCMB_dfw, header=FALSE, cov.std = FALSE)
#summary(SEM8_LCMB_dfw, fit.measures=TRUE)
# Uncomment below to produce graphs of SEM fits, but they do not "knit" in Rmd, hence graphics imported
##graph_sem(model = SEM8_LCMB_dfw)
#semPlot::semPaths(SEM8_LCMB_dfw, what = "paths", whatLabels = "est", intercepts = TRUE, edge.color = "black")
```

Note that we explicitly test whether the slope depends on the baseline in a final regression term, which it does here, because we generated data in which the rate of decline in the brain increases with age (with LG[1,2]<0) ***(or we could just examine the covariance between these latent variables, for which Z value similar but not identical - any reason for using covariances vs regression? regression better because implied directionality?***)

Note that in proper data analyses, one should always check overall model fit (by setting fit.measures=TRUE in the summary command; convention suggests good fits have CFI/TLI $> 0.95$, RMSEA $< 0.05$, and SRMR $< 0.08$). This linear slope model does not fit particularly well, in contrast to the LCSM model in next section, which can capture the quadratic-like effects of lag (from the way these data were generated).

##### 2.3.2.1 LCM for Brain-Cognition change

We can now add cognition to test for slope-slope relationships and baseline-slope relationships. Note that we now have positive dependencies of cognition on brain (even though brain is decreasing with age), where high brain values also mean higher rates of change (slopes) in cognition:

```{r plot B+C}
plot_long(dfl, xlab="B", ylab="C", titletxt = "Cross-sectional and longitudinal effects of Brain on Cognition")
```

Having defined slope and baseline for brain and cognition separately, we can now add relationships between the two slopes and the two baselines (see Figure 5B for a graphical depiction):
...
C2 <- dfl$C[dfl$wav==2]
C3 <- dfl$C[dfl$wav==3]
dfw = data.frame(B1, B2, B3, C1, C2, C3)
SEM8_LCMBC <- '
    B_bas =~ 1*B1 + 1*B2 + 1*B3
    B_slp =~ 0*B1 + 1*B2 + 2*B3
    C_bas =~ 1*C1 + 1*C2 + 1*C3
    C_slp =~ 0*C1 + 1*C2 + 2*C3
...
#    C_slp ~ B_slp
    ...
    C_slp ~~ B_slp
'            
SEM8_LCMBC_dfw <- lavaan::growth(SEM8_LCMBC, data = dfw)
# Uncomment below to produce graphs of SEM fits, but they do not "knit" in Rmd, hence graphics imported
##graph_sem(model = SEM8_LCMBC_dfw)
#semPlot::semPaths(SEM8_LCMBC_dfw, what = "paths", whatLabels = "est", intercepts = TRUE, edge.color = "black")
```

 ***uncommenting the C_bas-B_bas and C_slp-B_slp ~ regressions, instead of the ~~ covariance relationship, lead to estimation errors, so kept as covariance relationship(plus semplot better!!!)*** 
 
When we fit this model, we can see that the C slope depends on the B baseline, but not vice versa: 

```{r LMC B+C summary}
summary(SEM8_LCMBC_dfw, header=FALSE, cov.std = FALSE)
```

This suggests that brain levels are driving cognitive change, as expected.

#### 2.2.3. Latent Change Score Models (LCSM)

Earlier, we defined new variables like ``dB`` as the difference across two successive measurements. In LCSMs, we treat these differences as latent variables. 

##### 2.2.3.1 LCSM for Brain change only

To illustrate this, we will again begin with modelling just the three brain measures (B1-B3). The graph is shown in Figure 6A, where we define two difference scores across B1-B2 and B2-B3, while the lavaan code for such a LCSM is below (the comments in code below explain purpose of each line):

![Figure 6. A: LCSM for Brain change (dashed lines are fixed; intercepts in triangles). B: LCSM for Brain-Cognition change (intercepts now not shown).](./Fig6.jpg) 

&nbsp;

```{r LCSM B}
SEM9_LCSMB <- '
# First change between B1 and B2
B2   ~ 1*B1          # This parameter regresses B2 perfectly on B1
dB1 =~ 1*B2          # Latent change score factor as measured perfectly by scores on B2
B2   ~ 0*1           # This constrains the intercept of B2 to 0
B2  ~~ 0*B2          # This fixes the variance of the B2 to 0 

dB1  ~ 1             # This estimates the intercept of the change score 
dB1  ~ B1            # This estimates how change depends on baseline
dB1 ~~ dB1           # This estimates the variance of the change score

# Second change between B2 and B3
B3   ~ 1*B2          # This parameter regresses B3 perfectly on B2
dB2 =~ 1*B3          # Latent change score factor as measured perfectly by scores on B3
B3   ~ 0*1           # This constrains the intercept of B3 to 0
B3  ~~ 0*B3          # This fixes the variance of the B3 to 0 

dB2  ~ 1             # This estimates the intercept of the change score 
dB2  ~ B2            # This estimates how change depends on baseline
dB2 ~~ dB2           # This estimates the variance of the change score

# Initial (baseline) values
B1   ~ 1             # This estimates the intercept of B1 
B1  ~~ B1            # This estimates the variance of B1 
'
SEM9_LCSMB_dfw <- lavaan(SEM9_LCSMB, data=dfw) 
# Uncomment below to produce graphs of SEM fits, but they do not "knit" in Rmd, hence graphics imported
#semPlot::semPaths(SEM9_LCSMB_dfw, what = "paths", whatLabels = "est", intercepts = TRUE, edge.color = "black")
##graph_sem(model = SEM9_LCSMB_dfw)
```

The corresponding path model looks complicated, but it is essentially defining new latent variables dB1 and dB2 ("change scores"), about which we want to make inferences (along with the latent variable B1, and as a consequence, B2 and B3 are fixed). For example, while we could have just done a T-test to see if the slope dB1 is significantly different from 0, the power of this LCSM approach is that we can also infer 1) that its variance is significant (i.e., that individuals differ in their amount of change), and 2) that the amount of change depends on the initial, baseline value (e.g. the significant regression of dB1 on B1). See [Kievit et al. 2018](https://doi.org/10.1016/j.dcn.2017.11.007) for fuller explanation. 

If you look at the results, including the overall model fit: 

```{r LCSM B summary}
summary(SEM9_LCSMB_dfw, header=FALSE, fit.measures = TRUE, cov.std = FALSE)
#summary(SEM9_LCSMB_dfw, fit.measures = TRUE, header=FALSE)
```

you can see that the baseline of B1 is close to 100 (as determined by the data-generating function above), while the negative slopes are significant, but non-identical (see below) ***but why less negative for dB2 than dB1 if decline accelerating?***. The paths from B1 to dB1, and from B2 to dB2 are significantly positive, consistent with more negative slopes with higher age. Finally the variances of dB1 and dB2 are different, suggesting participants differ in their slopes across successive timepoints.

Note that the overall model fit is better than the LCM above. The reason for this is that the LCM assumes a single (linear) slope across the three timepoints, whereas the LCSM allows the slopes of each pairwise difference to differ. We can test the latter by constraining the slopes for dB1 and dB2 to be identical, and compare this "equated" model to the full model above. The Chi^2 test (and AIC, BIC) shows that the full model (where the slopes can differ) is better (because the data were generated such that the change depends on the current level):
...
dB1  ~ a*1           # Intercept of the change score now constrained to match dB2 below
dB1  ~ B1            # This estimates how change depends on baseline
dB1 ~~ dB1           # This estimates the variance of the change score
...
dB2  ~ a*1           # Intercept of the change score now constrained to match dB1 above
dB2  ~ B2            # This estimates how change depends on baseline
dB2 ~~ dB2           # This estimates the variance of the change score
...
B1  ~~ B1            # This estimates the variance of B1 
'
SEM9_LCSMB_dfw_eqtd <- lavaan(SEM9_LCSMB_eqtd, data=dfw) 
#summary(SEM9_LCSMB_dfw_eqtd, header=FALSE, fit.measures = FALSE, cov.std = FALSE)
anova(SEM9_LCSMB_dfw, SEM9_LCSMB_dfw_eqtd)
```

(a similar approach can be used to test whether the variance in the slopes dB1 and dB2 differ significantly across timepoints, etc).

##### 2.3.2.2. LCSM for Brain-Cognition change

Nonetheless, the real power of LCSM models in the current context comes when we have more than one longitudinal variable. Here we revert back to just two timepoints, to simplify the model structure. In the first model, we assume that cognitive baseline and change depends on brain baseline and change (last three lines in model definition below):
...
# Within-C changes
C2   ~ 1*C1         # This parameter regresses C2 perfectly on C1
dC1 =~ 1*C2         # Latent change score factor as measured perfectly by scores on C2
C2   ~ 0*1          # This constrains the intercept of C2 to 0
C2  ~~ 0*C2         # This fixes the variance of the C2 to 0 

dC1  ~ 1             # This estimates the intercept of the change score 
dC1  ~ C1            # This estimates how change depends on baseline
dC1 ~~ dC1           # This estimates the variance of the change score

C1   ~ 1             # This estimates the intercept of C1 
C1  ~~ C1            # This estimates the variance of B1 

# Between B-C changes
C1  ~ B1             # Cognitive intercept depends on brain intercept
dC1 ~ dB1            # Cognition change depends on brain change
dC1 ~ B1             # Cognition change depends on baseline brain
'
SEM9_LCSMBC_dfw <- lavaan(SEM9_LCSMBC, data=dfw)
# Uncomment below to produce graphs of SEM fits, but they do not "knit" in Rmd, hence graphics imported
##graph_sem(model = SEM9_LCSMBC_dfw)
#semPlot::semPaths(SEM9_LCSMBC_dfw, what = "paths", whatLabels = "est", intercepts = TRUE, edge.color = "black")
```

The between-domain brain-cognition relationships include 1) dependency of C baseline on B baseline (cross-sectional), 2) dependency of C slope on B slope (change-change longitudinal) and 3) dependency of C slope on B baseline ("lead-lag" dependency) -  see Figure 6B. If we examine the coefficients:

```{r LCSM B->C summary}
summary(SEM9_LCSMBC_dfw, header=FALSE, cov.std = FALSE, fit.measures = TRUE)
```

we find positive baseline-baseline and slope-slope dependency, though a negative slope-baseline dependency. ***why is this - shouldn't slope-baseline be positive too - eg in plot above, larger cognitive changes for higher values of B???***

***also, if add dB1 ~ C1, then model fit even better, and the dB1 ~ C1 is significant (albeit small), even though shouldn't be??***

We can then compare this model to a "reversed" model, where cognitive baseline and slope drive brain baseline and slope instead. This is certainly plausible in real life (e.g, cognitive ability might influence lifestyle choices that affect the brain health), but we know this was not how the data were created in these simulations, so we expect the reversed model to fit worse:
...
# Between B-C changes
B1  ~ C1             # Brain intercept depends on cognitive intercept
dB1 ~ dC1            # Brain change depends on cognitive change
dB1 ~ C1             # Brain change depends on baseline cognitive
'
SEM9_LCSMBC_dfw_rev <- lavaan(SEM9_LCSMBC_rev, data=dfw)
#summary(SEM9_LCSMBC_dfw_rev,  header=FALSE, cov.std = FALSE, fit.measures = TRUE)
anova(SEM9_LCSMBC_dfw, SEM9_LCSMBC_dfw_rev)
```

which it does, i.e, "rev" model has higher AIC/BIC and Chisq error. This provides evidence for the causal direction being from brain to cognition rather than vice versa.

### 2.4. Summary of longitudinal SEMs

Note we have only considered a single measure of each variable at each timepoint, but it is easy to extend LCSMs like above to include latent factors (e.g, of Brain and Cognition) that are estimated from multiple measurements at each timepoint (and be able to test for measurement invariance, as Section 1 above, but now across time within an individual, rather than across individuals in different age groups).

More generally, returning to the question of ways to model effects of Age, note that we no longer need to explicitly include Age in the model, since the effect of Age(ing) is captured by the change in those variables. (We could add Age as an input into the baseline brain variable (B1) but this would not affect the type of conclusions above about effects of Brain on Cognition - it is sufficient that the changes in Brain (which we happen to know are due to ageing) that drive the changes in cognition.)
